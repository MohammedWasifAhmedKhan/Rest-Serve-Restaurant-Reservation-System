...
import {AppColors, AppFontSize, GlobalStyles, WINDOW_WIDTH} from '../Global'; // Importing custom global styles and constants. 
...
export default function ForgotPassword(props: any) { // Declaring ForgotPassword functional component. Props are passed as argument to this component.
  ...
  return (
    <View style={[GlobalStyles.mainView, GlobalStyles.alignCenter]}> {/* [1] Rendering a View with global styles. */}
      <Text style={GlobalStyles.mainLabel}>Forgot Password</Text> {/* [7] Rendering text with global styles. */}
      <TextInputComp label={'Enter Email'} placeholder={'Enter Email'} /> {/* [3] Rendering custom TextInput component. */}
      <MainButton label={'Reset Password'} loading={false} onPress={handleLogin} /> {/* Rendering custom MainButton component. */}
      { /* Concept of event handling is implemented in above code line through the use of the onPress event for MainButton.*/ }
    </View>
  );
     { /* UI elements are rendered (rendering concept) within the return statement above.*/ }
}
